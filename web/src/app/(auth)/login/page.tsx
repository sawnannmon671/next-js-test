...
export default function LoginPage() {
  ...
  return (
    ...
    >
      {/* Outer card */}
      <div className="relative w-full max-w-[780px] rounded-[28px] overflow-hidden shadow-[0_30px_80px_rgba(0,0,0,0.1)]"
        style={{ background: "#ffffff", border: "1px solid #e5e7eb" }}
      >
        {/* Decorative leaf shapes — top left */}
        <div className="absolute top-0 left-0 w-[220px] h-[280px] pointer-events-none select-none" style={{ opacity: 0.1 }}>
          {/* Back leaf — teal */}
          <div className="absolute top-[-20px] left-[-30px] w-[180px] h-[240px] rounded-[50%_10%_50%_10%] rotate-[-15deg]"
            style={{ background: "linear-gradient(145deg, #0db4cc, #0891b2)", opacity: 0.2 }}
          />
          {/* Middle leaf — blue */}
          <div className="absolute top-[10px] left-[20px] w-[160px] h-[210px] rounded-[50%_10%_50%_10%] rotate-[10deg]"
            style={{ background: "linear-gradient(145deg, #3b5bdb, #1e3a8a)", opacity: 0.2 }}
          />
          {/* Front leaf — dark teal */}
          <div className="absolute top-[50px] left-[-10px] w-[130px] h-[180px] rounded-[50%_10%_50%_10%] rotate-[-5deg]"
            style={{ background: "linear-gradient(145deg, #0e9488, #0d7270)", opacity: 0.2 }}
          />
        </div>
        ...
        <div className="relative z-10 flex flex-col items-center py-14 px-10">
          ...
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-blue-800 text-3xl font-black tracking-tight">Approval<span style={{ color: "#22d3ee" }}>Sys</span></span>
          </div>
          <p className="text-sm mb-10" style={{ color: "#64748b" }}>Fast &amp; Easy Approval Management</p>

          <h1 className="text-blue-800 text-3xl font-bold mb-10 tracking-wide">Welcome Back!</h1>

          {/* Error banner */}
          {error && (
            <div className="w-full max-w-[320px] mb-6 px-4 py-3 rounded-xl text-xs font-bold text-red-600 bg-red-50 border-l-4 border-red-500">
              {error}
            </div>
          )}
          ...
          <form onSubmit={handleLogin} className="w-full max-w-[320px] space-y-8">
            {/* Email */}
            <div>
              <label className="text-xs font-semibold mb-2 block" style={{ color: "#64748b" }}>Email <span style={{ color: "#ef4444" }}>*</span></label>
              <div className="relative flex items-center">
                <input
                  type="email"
                  value={email}
                  onChange={(e) => setEmail(e.target.value)}
                  className="w-full bg-transparent pb-2 text-gray-900 text-sm font-medium focus:outline-none"
                  style={{ borderBottom: "1.5px solid #e2e8f0", background: "transparent" }}
                  placeholder=""
                  required
                />
                {isEmailValid && (
            ...
            {/* Password */}
            <div>
              <label className="text-xs font-semibold mb-2 block" style={{ color: "#64748b" }}>Password <span style={{ color: "#ef4444" }}>*</span></label>
              <div className="relative flex items-center">
                <input
                  type={showPassword ? "text" : "password"}
                  value={password}
                  onChange={(e) => setPassword(e.target.value)}
                  className="w-full bg-transparent pb-2 pr-8 text-gray-900 text-sm font-medium focus:outline-none"
                  style={{ borderBottom: "1.5px solid #e2e8f0", background: "transparent" }}
                  placeholder=""
                  required
                />
                <button
                  ...
                  style={{ color: showPassword ? "#22d3ee" : "#64748b" }}
                >
                  {showPassword ? (
          ...
          {/* Forgot */}
          <button type="button" className="mt-6 text-sm font-medium transition-colors hover:text-blue-800"
            style={{ color: "#64748b" }}>
            Forget My Password
          </button>

          {/* Footer links */}
          <div className="mt-16 flex items-center gap-3 text-xs" style={{ color: "#94a3b8" }}>
        ...
        <div className="absolute bottom-6 right-6 flex flex-col items-end gap-2">
          ...
          <button className="px-5 py-2.5 border border-gray-300 rounded-xl text-gray-700 text-xs font-bold shadow-lg hover:bg-gray-50 transition-all">
            Need Help?
          </button>
        </div>
      </div>
