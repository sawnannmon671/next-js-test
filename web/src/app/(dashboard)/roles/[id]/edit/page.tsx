      ...
      try {
        ...
        if (rolesResponse.success) {
          ...
          if (role) {
            ...
            // Assume role has permission_ids array
            setSelectedPermissionIds(role.permission_ids || []);
          } else {
            setGlobalError("Role not found");
          }
        } else {
          setGlobalError(rolesResponse.error || "Failed to load role");
        }
      } catch (error: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setGlobalError(response.error || "Failed to update role");
      }
    } catch (error: any) {
      setGlobalError(error.message);
    } finally {
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <main className="flex-1 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="mt-4 text-gray-500 font-medium">Loading role data...</p>
            </div>
          </main>
        </div>
      </div>
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="mb-12 space-y-4">
               ...
               <div className="flex justify-between items-end">
                  <div className="space-y-2">
                     <h1 className="text-5xl font-black text-gray-900 tracking-tight leading-none">Edit Role</h1>
                     <p className="text-gray-500 font-medium italic">Modify role configuration and permissions.</p>
                  </div>
               </div>
            </div>
            ...
            <div className="bg-white rounded-[1.5rem] shadow-[0_45px_100px_-25px_rgba(0,0,0,0.04)] border border-gray-100 overflow-hidden">
               <div className="bg-[#15aabf] p-10 text-white relative">
                  ...
                  <h2 className="text-3xl font-black tracking-tight relative z-10">Role Configuration</h2>
               ...
               <form onSubmit={handleSubmit} className="p-12 space-y-12">
                  ...
                  <div className="space-y-8">
                     <div className="flex items-center gap-4 border-b border-gray-100 pb-4">
                        ...
                        <h3 className="text-[10px] font-black uppercase tracking-[0.4em] text-gray-400">01 Role Information</h3>
                     </div>
                  ...
                  <div className="space-y-8">
                     <div className="flex items-center gap-4 border-b border-gray-100 pb-4">
                        ...
                        <h3 className="text-[10px] font-black uppercase tracking-[0.4em] text-gray-400">02 Assigned Permissions</h3>
                     </div>
                     <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        {permissions.map((permission) => (
                          <div key={permission.id} className="flex items-center gap-3 p-4 bg-gray-50 rounded-xl border border-gray-100 hover:border-[#15aabf]/30 transition-all cursor-pointer" onClick={() => handlePermissionToggle(permission.id)}>
                            <input
                              type="checkbox"
                            ...
                            />
                            <div>
                              <span className="text-sm font-bold text-gray-800 block">{permission.name}</span>
                              <span className="text-[10px] font-mono text-gray-500">{permission.code}</span>
                            </div>
                          </div>
                        ))}
                     </div>
